...
describe('CookieOption', () => {
  ...
  it('should have a non-necessary purpose type', () => {
    const cookieOption = CookieOption.create({ name: 'Necesseray cookie', necessaryType: false })
  ...
  it('should returns a 13 duration if the cookie is provided a duration', () => {
    const cookieOption = CookieOption.create({ name: 'Persistent cookie', necessaryType: true, duration: 13 })

    expect(cookieOption.duration()).toBe(13)
  })

  it('should not be instanciated if the duration is negative', () => {
    expect(() => CookieOption.create({ name: 'Persistent cookie', necessaryType: true, duration: -12 })).toThrowError(InvalidDurationError)
  })

  it('should execute a callback for an accepted option', () => {
    const cookieOption = CookieOption.create({ name: "Accepted cookie" })

    cookieOption.whenAccepted(() => { return "this cookie option has been accepted" })
  ...
  it('should execute a callback for a refused option', () => {
    ...
    cookieOption.whenRefused(() => { return "this cookie option has been refused" })
